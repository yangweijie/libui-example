...
class SnakeGame
{
    ...
    private function drawBackground(CData $drawParams): void
    {
        ...
        Draw::pathEnd($bgPath);
        Draw::fill($drawParams, $bgPath, $bgBrush);
    }
    ...
    private function drawGrid(CData $drawParams): void
    {
        ...
        for ($x = 0; $x <= $this->width; $x++) {
            ...
            Draw::pathEnd($path);
            Draw::Stroke($drawParams, $path, $gridBrush, $gridParams);
        }
    ...
    private function drawFood(CData $drawParams): void
    {
        ...
        Draw::fill($drawParams, $foodPath, $foodBrush);
    }
    ...
    private function drawSnake(CData $drawParams): void
    {
        ...
        foreach ($this->snake as $index => $segment) {
            ...
            if ($index == 0) {
                ...
                Draw::fill($drawParams, $headPath, $headBrush);
            } else {
                // 蛇身
                ...
                Draw::fill($drawParams, $segmentPath, $snakeBrush);
            }
        }
    }
     ...
     * @return void
     */
    private function drawUI(CData $drawParams): void
    {
        // 创建文本参数
        $fontParams = Draw::createFontParams();
        Draw::setFontFamily($fontParams, "Arial");
        Draw::setFontSize($fontParams, 16);

        $textParams = Draw::createTextParams();
        Draw::setTextParamsFont($textParams, $fontParams);
        Draw::setTextParamsColor($textParams,
            $this->colors['text'][0],
            $this->colors['text'][1],
            $this->colors['text'][2],
            $this->colors['text'][3]
        ...
        Draw::setTextParamsString($textParams, $scoreText);

        $textLayout = Draw::createTextLayout($textParams);
        Draw::text($drawParams, $textLayout, 10, 10);

        // 如果游戏结束，显示游戏结束文本
        if ($this->gameOver) {
            $gameOverParams = Draw::createFontParams();
            Draw::setFontFamily($gameOverParams, "Arial");
            Draw::setFontSize($gameOverParams, 32);

            $gameOverTextParams = Draw::createTextParams();
            Draw::setTextParamsFont($gameOverTextParams, $gameOverParams);
            Draw::setTextParamsColor($gameOverTextParams,
                $this->colors['text'][0],
                $this->colors['text'][1],
                $this->colors['text'][2],
                $this->colors['text'][3]
            );
            Draw::setTextParamsString($gameOverTextParams, "GAME OVER");

            $gameOverLayout = Draw::createTextLayout($gameOverTextParams);

            // 计算居中位置
            $layoutWidth = 0;
            $layoutHeight = 0;
            Draw::textLayoutExtents($gameOverLayout, $layoutWidth, $layoutHeight);

            $x = ($this->width * self::CELL_SIZE - $layoutWidth) / 2;
            $y = ($this->height * self::CELL_SIZE - $layoutHeight) / 2;

            Draw::text($drawParams, $gameOverLayout, $x, $y);
        }
    }
